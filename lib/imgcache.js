...
(function ($) {
  ...
  Helpers.fileGetExtension = function (filename) {
    ...
    var ext = filename
      .split('.')
      .pop();
    // make sure it's a realistic file extension - for images no more than 4
    // characters long (.jpeg)
    if (!ext || ext.length > 4) {
      return '.jpeg';
    }
    return ext;
  ...
  Helpers.appendPaths = function (path1, path2) {
    ...
    return path1 + (((path1[path1.length - 1] == '/') || (path2.length > 0 && path2[0] == '/'))
      ? ''
      : '/') + path2;
  };
  ...
  Helpers.EntryGetURL = function (entry) {
    ...
    return (typeof entry.toURL === 'function'
      ? Helpers.EntryToURL(entry)
      : entry.toURI());
  };
  ...
  Helpers.EntryGetPath = function (entry) {
    if (Helpers.isCordova()) {
      ...
      // From Cordova 3.3 onward toURL() seems to be required instead of fullPath
      // (#38)
      return (typeof entry.toURL === 'function'
        ? Helpers.EntryToURL(entry)
        : entry.fullPath);
      ...
      if (Helpers.isCordovaWindowsPhone() || !window.CustomEvent) {
        ...
        window.CustomEvent = function CustomEvent(type, params) {
          ...
          try {
            ...
            // for browsers that don't support CustomEvent at all, we use a regular event
            // instead
  ...
  DomHelpers.getBackgroundImage = function (element) {
    if (Helpers.hasJqueryOrJqueryLite()) {
      return element.attr('data-old-background')
        ? "url(" + element.attr('data-old-background') + ")"
      ...
      return element.getAttribute("data-old-background")
        ? "url(" + element.getAttribute("data-old-background") + ")"
  ...
  Private.isImgCacheLoaded = function () {
    if (!ImgCache.attributes.filesystem || !ImgCache.attributes.dirEntry) {
      ImgCache
        .overridables
        .log('ImgCache not loaded yet! - Have you called ImgCache.init() first?', LOG_LEVEL_WARNING);
      return false;
    }
    return true;
  };
  ...
  Private.hasLocalStorage = function () {
    ...
    try {
      ...
      ImgCache
        .overridables
        .log('Could not write to local storage: ' + e.message, LOG_LEVEL_INFO);
      return false;
    }
  };
  ...
  Private.setCurrentSize = function (curSize) {
    ImgCache
      .overridables
      .log('current size: ' + curSize, LOG_LEVEL_INFO);
  ...
  Private.getCachedFileName = function (img_src) {
    if (!img_src) {
      ImgCache
        .overridables
        .log('No source given to getCachedFileName', LOG_LEVEL_WARNING);
      return;
    }
    var hash = ImgCache
      .overridables
  ...
  Private.createCacheDir = function (success_callback, error_callback) {
    if (!ImgCache.attributes.filesystem) {
      ImgCache
        .overridables
        .log('Filesystem instance was not initialised', LOG_LEVEL_ERROR);
      if (error_callback) {
        error_callback();
      }
      return;
    ...
    var _fail = function (error) {
      ImgCache
        .overridables
        .log('Failed to get/create local cache directory: ' + error.code, LOG_LEVEL_ERROR);
      if (error_callback) {
        error_callback();
      }
    };
    var _getDirSuccess = function (dirEntry) {
      ...
      ImgCache
        .overridables
        .log('Local cache folder opened: ' + Helpers.EntryGetPath(dirEntry), LOG_LEVEL_INFO);
      ...
      if (Helpers.isCordovaAndroid()) {
        var _androidNoMediaFileCreated = function () {
          ImgCache
            .overridables
            .log('.nomedia file created.', LOG_LEVEL_INFO);
          if (success_callback) {
            success_callback();
          }
        };
        ...
        if (Helpers.isCordovaIOS() && dirEntry.setMetadata) {
          dirEntry
            .setMetadata(function () {
              ...
              ImgCache
                .overridables
                .log('com.apple.MobileBackup metadata set', LOG_LEVEL_INFO);
            }, function () {
              /* failure */
              ImgCache
                .overridables
                .log('com.apple.MobileBackup metadata could not be set', LOG_LEVEL_WARNING);
            }, {
    ...
    ImgCache
      ...
      .filesystem
      .root
      .getDirectory(ImgCache.options.localCacheFolder, {
        create: true,
        exclusive: false
      }, _getDirSuccess, _fail);
  };

  // This is a wrapper for phonegap's FileTransfer object in order to implement
  // the same feature in Chrome (and possibly extra browsers in the future)
  ...
  Private.FileTransferWrapper.prototype.download = function (uri, localPath, success_callback, error_callback, on_progress) {
    ...
    if (this.fileTransfer) {
      ...
      return this
        .fileTransfer
        .download(uri, localPath, success_callback, error_callback, false, {'headers': headers});
    }
    ...
    var _fail = function (str, level, error_callback) {
      ...
      // Normally, the error.code in the callback is a FileWriter error, we return 0
      // if the error was an XHR error
      if (error_callback) {
  ...
  Private.loadCachedFile = function ($element, img_src, set_path_callback, success_callback, error_callback) {
    ...
    var _gotFileEntry = function (entry) {
      if (ImgCache.options.useDataURI) {
        var _win = function (file) {
          ...
          reader.onloadend = function (e) {
            ...
            if (!base64content) {
              ImgCache
                .overridables
                .log('File in cache ' + filename + ' is empty', LOG_LEVEL_WARNING);
              if (error_callback) {
            ...
            ImgCache
              .overridables
              .log('File ' + filename + ' loaded from cache', LOG_LEVEL_INFO);
            if (success_callback) {
        ...
        var _fail = function (error) {
          ImgCache
            .overridables
            .log('Failed to read file ' + error.code, LOG_LEVEL_ERROR);
          if (error_callback) {
            error_callback($element);
          }
        };
    ...
    var _fail = function () {
      ImgCache
        .overridables
        .log('File ' + filename + ' not in cache', LOG_LEVEL_INFO);
      if (error_callback) {
        error_callback($element);
      }
    };
    ImgCache
      ...
      .filesystem
      .root
      .getFile(Private.getCachedFilePath(img_src), {
        create: false
      }, _gotFileEntry, _fail);
  };
  ...
  ImgCache.init = function (success_callback, error_callback) {
    ImgCache.jQuery = (window.jQuery || window.Zepto)
      ? true
      : false;/* using jQuery if it's available otherwise the DOM API */
    ImgCache.jQueryLite = (typeof window.angular !== 'undefined' && window.angular.element)
      ? true
      : false;/* is AngularJS jQueryLite available */

    ImgCache.attributes.init_callback = success_callback;

    ImgCache
      .overridables
      .log('ImgCache initialising', LOG_LEVEL_INFO);
    ...
    var _gotFS = function (filesystem) {
      ImgCache
        .overridables
        .log('LocalFileSystem opened', LOG_LEVEL_INFO);
    ...
    var _fail = function (error) {
      ImgCache
        .overridables
        .log('Failed to initialise LocalFileSystem ' + error.code, LOG_LEVEL_ERROR);
      if (error_callback) {
        error_callback();
      }
    };
    if (Helpers.isCordova() && window.requestFileSystem) {
      ...
      if (ImgCache.options.cordovaFilesystemRoot) {
        try {
          window
            .resolveLocalFileSystemURL(ImgCache.options.cordovaFilesystemRoot, function (dirEntry) {
      ...
      window.storageInfo = window.storageInfo || (ImgCache.options.usePersistentCache
        ? navigator.webkitPersistentStorage
        : navigator.webkitTemporaryStorage);
      if (!window.storageInfo) {
        ImgCache
          .overridables
          .log('Your browser does not support the html5 File API', LOG_LEVEL_WARNING);
        if (error_callback) {
          error_callback();
        }
        return;
      ...
      window
        .storageInfo
        .requestQuota(quota_size, function () {
          /* success*/
          var persistence = (ImgCache.options.usePersistentCache
            ? window.PERSISTENT
            : window.TEMPORARY);
          ...
          ImgCache
            .overridables
            .log('Failed to request quota: ' + error.message, LOG_LEVEL_ERROR);
          if (error_callback) {
            error_callback();
          }
        });
  ...
  // this function will not check if the image is already cached or not => it will
  // overwrite existing data on_progress callback follows this spec:
  ...
  ImgCache.cacheFile = function (img_src, success_callback, error_callback, on_progress) {
    ...
    fileTransfer.download(img_src, filePath, function (entry) {
      entry
        .getMetadata(function (metadata) {
          if (metadata && ('size' in metadata)) {
            ImgCache
              .overridables
              .log('Cached file size: ' + metadata.size, LOG_LEVEL_INFO);
            Private.setCurrentSize(ImgCache.getCurrentSize() + parseInt(metadata.size, 10));
          } else {
            ImgCache
              .overridables
              .log('No metadata size property available', LOG_LEVEL_INFO);
          }
        });
      ImgCache
        .overridables
        .log('Download complete: ' + Helpers.EntryGetPath(entry), LOG_LEVEL_INFO);

      // iOS: the file should not be backed up in iCloud new from cordova 1.8 only
      ...
      if (error.source) {
        ImgCache
          .overridables
          .log('Download error source: ' + error.source, LOG_LEVEL_ERROR);
      }
      if (error.target) {
        ImgCache
          .overridables
          .log('Download error target: ' + error.target, LOG_LEVEL_ERROR);
      }
      ImgCache
        .overridables
        .log('Download error code: ' + error.code, LOG_LEVEL_ERROR);
      if (error_callback) {
        error_callback();
      }
    }, on_progress);
  ...
  // Returns the file already available in the cached Reminder: this is an
  // asynchronous method! Answer to the question comes in response_callback as the
  // second argument (first being the path)
  ...
  // checks if a copy of the file has already been cached Reminder: this is an
  // asynchronous method! Answer to the question comes in response_callback as the
  // second argument (first being the path)
  ImgCache.isCached = function (img_src, response_callback) {
    ImgCache
      .getCachedFile(img_src, function (src, file_entry) {
        response_callback(src, file_entry !== null);
      });
  };

  // $img: jQuery object of an <img/> element Synchronous method
  ...
  ImgCache.clearCache = function (success_callback, error_callback) {
    ...
    ImgCache
      ...
      .removeRecursively(function () {
        ImgCache
          .overridables
          .log('Local cache cleared', LOG_LEVEL_INFO);
        ...
        ImgCache
          .overridables
          .log('Failed to remove directory or its contents: ' + error.code, LOG_LEVEL_ERROR);
        if (error_callback) {
          error_callback();
        }
      });
  ...
  ImgCache.removeFile = function (img_src, success_callback, error_callback) {
    ...
    var _fail = function (error) {
      ImgCache
        .overridables
        .log('Failed to remove file due to ' + error.code, LOG_LEVEL_ERROR);
      if (error_callback) {
        error_callback();
      }
    };
  ...
  ImgCache.cacheBackground = function ($div, success_callback, error_callback, on_progress) {
    ...
    if (!img_src) {
      ImgCache
        .overridables
        .log('No background to cache', LOG_LEVEL_WARNING);
      if (error_callback) {
        error_callback();
      }
      return;
    ...
    ImgCache
      .overridables
      .log('Background image URL: ' + img_src, LOG_LEVEL_INFO);
  ...
  // $div: jQuery object of an element Synchronous method Method used to revert
  // call to useCachedBackground
  ...
  // returns the URI of the local cache folder (filesystem:) this function is more
  // useful for the examples than for anything else.. Synchronous method
